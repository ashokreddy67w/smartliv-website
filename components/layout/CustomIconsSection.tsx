...
import {
  ...
  Home,
  Activity,
  Sun, // Keep the import but use it below
} from "lucide-react";

// Sun icon component (using lucide-react's Sun instead of custom SVG)
// If you want to keep the custom SVG, rename the component to SunIcon
...
const iconExamples = [
  ...
  {
    icon: Sun, // Using lucide-react's Sun component
    label: "Good Morning",
    ...
    <section className="bg-black py-32 md:py-40" aria-labelledby="custom-icons-heading">
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 lg:px-16">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          {/* Image - Replaced with real product image */}
          <motion.div
            initial={{ opacity: 0, x: -40 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}
            className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] aspect-[4/5]"
          >
            <Image
              src="/images/smartliv-custom-icons-switch.webp"
            ...
            >
              <div className="bg-white/10 backdrop-blur-xl px-4 py-3 rounded-2xl border border-white/10">
                <div className="text-brand-green text-[10px] tracking-widest uppercase mb-0.5">Laser Engraved</div>
                <div className="text-white text-sm font-light">500+ premium smart home icons</div>
              </div>
            </motion.div>
          </motion.div>
          ...
          <div>
            ...
            <Reveal delay={0.1}>
              <h2 id="custom-icons-heading" className="text-headline font-thin text-white mb-6">
                ...
                {iconExamples.map(({ icon: Icon, label, description }, index) => (
                  <StaggerItem key={label}>
                    <motion.div
                      ...
                      className="relative flex flex-col items-center gap-2 p-3 rounded-xl glass hover:glass-green cursor-pointer transition-all duration-200 group"
                      tabIndex={0}
                    >
                      <Icon size={20} className="text-white/40 group-hover:text-brand-green transition-colors" aria-hidden="true" />
            ...
            <Reveal delay={0.5}>
              <p className="text-white/20 text-sm font-light mt-6">
                ...
                <a 
                  href="/custom-icons" 
                  className="text-brand-green hover:underline transition-all hover:gap-2 inline-flex items-center group"
                >

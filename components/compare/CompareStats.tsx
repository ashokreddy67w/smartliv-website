// components/compare/CompareStats.tsx
'use client';
...
const stats = [
  { value: 10000, suffix: '+', label: 'Homes Automated' },
  { value: 1500, suffix: '+', label: 'Projects Delivered' },
  { value: 99.9, suffix: '%', label: 'System Reliability' },
...
function AnimatedCounter({ end, duration = 2500 }: { end: number; duration?: number }) {
  ...
  useEffect(() => {
    ...
    const element = document.getElementById(`compare-stat-${end}`);
    ...
    >
      {stats.map((stat) => (
        <div key={stat.label} className="text-center">
          <div className="text-3xl md:text-4xl font-bold text-white tracking-tight" id={`compare-stat-${stat.value}`}>
